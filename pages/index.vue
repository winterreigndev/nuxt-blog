<template>
  <div>
    <div class="sort-count">
      <div>
        <span class="showing-articles">Articles:
          <span class="no-focus">
            <input v-model="searchQuery" style="background: transparent" tabindex="-1" readonly="readonly"
              placeholder="All" />
          </span>
        </span>
      </div>
      <div>
        <span class="sort-articles">
          Search Articles For:
          <input v-model="searchQuery" type="search" autocomplete="off" placeholder="Type Here" />
        </span>
      </div>
        ...
        <div v-if="results.length" class="grid-two">
          <div v-for="result of results" :key="result.slug" class="width-100">
            <div class="articles grid-article">
              <div>
                <nuxt-link :to="{ name: 'blog-slug', params: { slug: result.slug } }">
                  <img :src="require(`~/assets/resources/${result.img}`)" alt="result.title" />
                </nuxt-link>
              </div>
              <div>
                <div class="date">{{ formatDate(result.createdAt) }}</div>
                <nuxt-link class="post-title" :to="{ name: 'blog-slug', params: { slug: result.slug } }">
                  {{ result.title }}
                </nuxt-link>
                <article>
                  {{ result.description | excerpt }} <i>...</i>
                </article>
        ...
        <div class="articles" v-else>
          ...
          <p><strong>Search Suggestions:</strong>
            <ul class="suggestions-list">
              ...
              <li>Try to use keywords rather than full sentences</li>
            </ul>
          </p>
        </div>
      </div>
          ...
          <div class="articles grid-article">
            <div>
              <nuxt-link :to="{ name: 'blog-slug', params: { slug: article.slug } }">
                <img :src="require(`~/assets/resources/${article.img}`)" alt="article.title" />
              </nuxt-link>
            </div>
            <div>
              <div class="date">{{ formatDate(article.createdAt) }}</div>
              <nuxt-link class="post-title" :to="{ name: 'blog-slug', params: { slug: article.slug } }">
                {{ article.title }}
              </nuxt-link>
              <article>
